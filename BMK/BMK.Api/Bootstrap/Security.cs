        ...
        private static void HandleException(this IApplicationBuilder applicationBuilder)
        {
            applicationBuilder.UseExceptionHandler(errorApp =>
            {
                errorApp.Run(async context =>
                {
                    ...
                    var logException = context.RequestServices.GetService(typeof(ILogException)) as ILogException;
                    var exception = errorFeature.Error;
                    var err = await logException.Log(errorFeature.Error, context.Request.Path.Value);
                    var traceId = await exception.LogAsync(context);
                    context.Response.ContentType = "application/json;";
                    await context.Response.WriteAsync("Error Has Occured.");//errorFeature.Error.Message
                });
            });
        }
